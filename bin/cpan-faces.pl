...
#my ( $data, $type ) = get_icon_new( 'andya' );
#print length( $data ), ", $type\n";
#exit;
...
sub build_page {
    ...
    return $h->html(
        [
            ...
            $h->body(
                [
                    $h->h2( 'Our Free Software Runs Your Company' ),
                    @pic
                ]
            )
        ]
...
sub save_icon {
    my $id = shift;
    my %ext_map = ( jpeg => 'jpg' );
    my ( $data, $type ) = eval { get_icon_new( $id ) };
...
sub get_icon_new {
    ...
    my $p    = HTML::TokeParser->new( \$body );
    while ( my $tok = $p->get_token ) {
        my ( $type, $tag, $attr, $attrseq, $text ) = @$tok;
